function Add-ExchangePsDatabaseDAGSensor{
    Param (
    ...
    if($logfile -eq "C:\Scripts\PRTG\Logs\LOG_Add-ExchangePsDatabaseDAGSensor.txt"){
        Add-content -path $logfile  -Value "$(get-date -Format "dd.MM.yyyy_HH.mm.ss"): -------------------------------Adding PS Exchange DB DAG sensor to $($device.name)-------------------------------"
    }
    else{

        Add-content -path $logfile  -Value "$(get-date -Format "dd.MM.yyyy_HH.mm.ss"): -------------------------------PS Exchange DB DAG sensor-------------------------------"
    ...
    try{
        $targets = $device | get-sensortarget -rawType exchangepsdatabasedag -Timeout 5000 -ErrorAction Stop 
    }
    catch{
    ...
    foreach($target in $targets){
        ...
        $table = @{
            "name_"="Base Sensor"
            "tags_"="exchange powershell database dag"
            "priority_"=3
            "elementlist_"=1
            ...
            "inherittriggers"=1
            "id"=$device.id
            "sensortype"="exchangepsdatabasedag"
        }
