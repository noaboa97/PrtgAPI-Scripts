...
function Find-PrtgUser {
    ...
    [CmdletBinding()]
        Param (
                ...
                [Parameter(mandatory = $true, HelpMessage = "PRTG Displayname", Position = 0,ParameterSetName="Displayname")]
                [string]$Displayname
    ...
    BEGIN{
        ...
        if($Displayname){
            ...
            $id = (get-object -Name Benutzer -Type System | get-object | Where-Object {$_.name -eq $Displayname}).id
        }
    ...
    PROCESS{
        ...
        Try{
            ...
            if ($null -eq $table) {
                ...
                foreach ($row in $rows) {
                    if($row.innertext -ne "Name der Benutzergruppe"){
