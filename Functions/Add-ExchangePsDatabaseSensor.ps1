function Add-ExchangePsDatabaseSensor{
    Param (
    ...
    if($logfile -eq "C:\Scripts\PRTG\Logs\LOG_Add-ExchangePsDatabaseSensor.txt"){
        Add-content -path $logfile  -Value "$(get-date -Format "dd.MM.yyyy_HH.mm.ss"): -------------------------------Adding PS Exchange DB sensor to $($device.name)-------------------------------"
    }
    else{

        Add-content -path $logfile  -Value "$(get-date -Format "dd.MM.yyyy_HH.mm.ss"): -------------------------------PS Exchange DB sensor-------------------------------"
    ...
    try{
        $targets = $device | get-sensortarget -rawType exchangepsdatabase -Timeout 5000 -ErrorAction Stop 
    }
    catch{
    ...
    foreach($target in $targets){
        ...
        $table = @{
            "name_"="Base Sensor"
            "tags_"="exchange powershell database"
            "priority_"=3
            "elementlist_"=1
